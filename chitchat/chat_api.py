...
def set_interact_args():
    ...
    parser.add_argument('--voca_path', default='dialogue_model/vocab.txt', type=str, required=False, help='选择词库')
...
tokenizer = BertTokenizer(vocab_file='./dialogue_model/vocab.txt')
model = GPT2LMHeadModel.from_pretrained('./dialogue_model')
model.to(device)
model.eval()


# from app.api.v1.chat import max_history_len, max_len, topk, topp, repetition_penalty, temperature


def chat(text,max_history_len, max_len, topk, topp, repetition_penalty, temperature):
    save_samples_path = './sample'  # 保存日志目录，是否需要保存聊天记录
